...
# Fonction pour obtenir le chemin du bureau de l'utilisateur
...
def detecter_image_et_lancer_script():
    ...
    if template is None:
        print(f"Erreur : Impossible de charger l'image de référence {template_path}")
        return
    ...
    try:
        ...
        if num_matches == 5:
            ...
            # Chercher l'image combat.png dans le répertoire du bureau de l'utilisateur
            desktop_path = get_user_desktop_path()
            combat_image_path = os.path.join(desktop_path, 'ankador', 'img', 'combat.png')
            
            print(f"Recherche de l'image combat.png dans : {combat_image_path}")
            
            # Recherche dynamique de l'image
            location = pyautogui.locateOnScreen(combat_image_path, confidence=0.8)
