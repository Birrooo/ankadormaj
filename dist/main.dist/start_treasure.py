...
def main():
    ...
    try:
        ...
        # Une fois tous les clics terminés, lancer le script go_zaap.py
        print("Tous les clics terminés, lancement de go_zaap.py...")

        # Utiliser get_resource_path pour obtenir le chemin complet du script go_zaap.py
        go_zaap_path = get_resource_path('go_zaap.py')

        # Vérifier si le fichier existe
        if go_zaap_path and os.path.exists(go_zaap_path):
            # Lancer le script go_zaap.py
            subprocess.run(["python", go_zaap_path])
        else:
            print("Erreur : Le fichier go_zaap.py est introuvable.")
